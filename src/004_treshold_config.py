...
SUCCESS_RATE_THRESHOLDS = [
    dashboard_model.Threshold(color="red", value=0.0),      # 0 - 0.6
    dashboard_model.Threshold(color="orange", value=60),   # 0.6 - 0.7
    dashboard_model.Threshold(color="yellow", value=70),   # 0.7 - 0.8
    dashboard_model.Threshold(color="green", value=80),    # > 0.8
]
...
def text_description() -> text_builder.Panel:
    return (
        text_builder.Panel()
        .title("Description")
        .content("Treshold config example")
...
def timeseries_http_status_codes() -> timeseries_builder.Panel:
    ...
    return (
        ...
        .thresholds(
            dashboard_builder.ThresholdsConfig()
            .mode(dashboard_model.ThresholdsMode.PERCENTAGE)
            .steps(SUCCESS_RATE_THRESHOLDS)
            
        )
        .thresholds_style(
            common_builder.GraphThresholdsStyleConfig()
            .mode(common_model.GraphThresholdsStyleMode.DASHED_AND_AREA)
        )
    )
